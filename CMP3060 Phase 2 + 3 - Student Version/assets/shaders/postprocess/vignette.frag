...
void main(){
    ...
    // The pixel's location in the NDC space is the pixel's texture coordinate times 2
    frag_color = texture(tex, tex_coord) / (1.0 + ((2.0 * tex_coord.x - 1.0) * (2.0 * tex_coord.x - 1.0)) + ((2.0 * tex_coord.y - 1.0) * (2.0 * tex_coord.y - 1.0)));
}